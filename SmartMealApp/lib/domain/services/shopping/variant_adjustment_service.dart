...
class VariantAdjustmentService {
  ...
  static double getVariantMultiplier(String ingredientName) {
    final name = ingredientName.toLowerCase();
      ...
      // Congelado: -10%
      if (name.contains('congelado') ||
        name.contains('frozen')) {
      return 0.90;
    }
    
      // Fresco: +5%
      if (name.contains('fresco') ||
        name.contains('fresh')) {
      return 1.05;
    }
  ...
  static List<String> detectVariants(String ingredientName) {
    ...
    if (name.contains('bio') || name.contains('ecológico') || name.contains('ecologico')) {
      variants.add('Ecológico (+20%)');
    }
    if (name.contains('congela')) { // "congelado", "congelada", "congeladas", etc.
    ...
    if (name.contains('premium') || name.contains('gourmet') || name.contains('ibérico')) {
